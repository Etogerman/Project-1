...
## Независимая проверка текущего HEAD

- Для code/runtime PR в `staging` и `main` проверяй только текущий commit SHA.
- Review draft PR является предварительным и не закрывает пакет независимых
  проверок; засчитывается только новая проверка текущего `HEAD` после `ready`.
- Выполняй собственную проверку diff, тестов и продуктовых инвариантов, не
  опираясь на summary или выводы другого ИИ-агента.
- Не считай прошлый review актуальным после появления нового commit.
- Если `HEAD` изменился во время review, явно укажи, что вывод относится к
  прежнему commit и требуется новая проверка текущего `HEAD`.
- Review считается частью пакета независимых проверок, но сводный технический
  вердикт после сопоставления обеих проверок даёт агент по правилам
