...
## Обязательное правило после смены маршрута

После любой смены `LINE_ID`, `connector_code`, callback owner,
`callback_base_url`, tunnel URL или локального route канала нужно заново
опубликовать OpenLines registry и выполнить Doctor до проверки обратного
сообщения из Bitrix24.

Это правило действует и для локального контура. Исправление локальной записи
канала в Laravel само по себе не обновляет route registry на стороне Bitrix24.
Если Bitrix24 продолжает слать событие в старый callback или показывает ошибку
доставки операторского ответа, первым делом проверяется registry.

Минимальный порядок проверки:

1. Убедиться, что в Bitrix24 profile есть секрет публикации registry.
2. Опубликовать registry для нужного profile и portal.
3. Запустить Doctor и получить:
...
4. Проверить, что server snapshot содержит актуальные:
...
5. Только после этого отправлять новый smoke из Bitrix24 операторским
   сообщением и проверять доставку клиенту.

Публикация route registry не создаёт новую открытую линию. Она только сообщает
managed-code коннектору Bitrix24, какой существующий `connector_code + LINE_ID`
должен ходить в какой callback owner.

Старые красные сообщения, отправленные до исправления registry, не являются
валидным smoke после фикса. Для проверки всегда отправляется новое операторское
сообщение.
