...
4. пользователь проверяет `CI`; агент выполняет обязательную авторскую
   самопроверку до `ready` только по отдельной команде пользователя
5. после зелёного `CI` и завершённой авторской самопроверки пользователь
   переводит staging PR в `Ready for review`, пишет агенту `готово, проведи
   независимое ревью` и передаёт тот же PR стороннему ИИ-ревьюеру
6. агент и сторонний ИИ-ревьюер параллельно и независимо проверяют один
   зафиксированный `HEAD`; новый commit обнуляет обе проверки и требует
   повторной пользовательской проверки CI и делегации обеим сторонам
7. после завершения обеих проверок агент читает внешний review, все актуальные
   comments/threads и `CI` того же `HEAD` и даёт сводный технический вердикт
...
11. в PR в `main` повторяется тот же порядок: пользовательская проверка `CI`,
    авторская самопроверка до `ready`, пользовательский `ready` и делегация,
    две параллельные независимые проверки одного `HEAD`, сводный вердикт агента
...
- обязательная авторская самопроверка до `ready`
...
- две параллельные независимые проверки одного зафиксированного `HEAD`
