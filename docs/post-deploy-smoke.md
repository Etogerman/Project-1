...
Успешный production smoke подтверждает технический результат, но не закрывает
stream автоматически.

Обязательный порядок:

1. Зафиксировать проверенный environment, release ref, результат и ограничения.
2. Пользователь или оператор принимает production-результат либо явно принимает
   риск.
3. Агент сверяет все Issues из `Связанные задачи` с принятым результатом.
4. Пользователь по каждой Issue решает и выполняет: `закрыть` или
   `оставить открытой`.
5. При `Связанные задачи: не требуется` фиксируется
   `Issue Closure: not_required`; оставленная открытой Issue становится
   `issue/admin tail`.
6. Выполняется применимый `Spec Closure` либо фиксируется
   `Spec Closure: not_required` с причиной.
7. Только после этого доступен cleanup.
