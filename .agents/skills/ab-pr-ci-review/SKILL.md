...
- отсутствие `Closes`, `Fixes` и `Resolves` для связанных задач до принятого
  production-результата;
...
- наличие `issue/admin tail`, если проверка выполняется после production-приёмки;
