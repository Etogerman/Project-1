...
blocker-ов для code/runtime PR следующий checkpoint — пользователь переводит PR
в ready, пишет `готово, проведи независимое ревью` или равнозначную команду и
передаёт PR стороннему ИИ-ревьюеру с требованием самостоятельно зафиксировать
...
- draft PR + CI зелёный + авторская самопроверка до ready не завершена -> агент выполняет авторскую самопроверку текущего снимка PR по отдельной команде пользователя;
- code/runtime draft PR + CI зелёный + авторская самопроверка завершена + поля готовности валидны + blocker-ов нет -> пользователь переводит PR в ready, явно делегирует агенту независимую проверку и передаёт PR стороннему ИИ-ревьюеру с требованием самостоятельно зафиксировать строку `Review snapshot`;
