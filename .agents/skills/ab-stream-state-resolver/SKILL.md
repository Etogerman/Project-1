...
4. модуль текущего состояния, если маршрутизатор его определил
5. `docs/task-delivery-workflow.md` только для этапа основного процесса, который
   ещё не вынесен в модуль
6. профильные документы по затронутой области
...
Для сигнала об ошибке в открытом PR не загружай весь delivery-playbook. Сначала
определи ID состояния, затем выполни
`node .github/scripts/workflow-docs-check.mjs --state <ID>` и прочитай только
возвращённый модуль.
...
Для docs-only/process-only PR после пользовательского `ready` применимые
проверки GitHub и выбранная проверка не возвращают текущую задачу Codex агенту
автоматически. Когда проверки завершились, пользователь передаёт задачу агенту
...
переход остаётся у пользователя; после команды — у агента. После команды агент
проверяет актуальное состояние текущего снимка PR, CI, выбранной проверки и всех
