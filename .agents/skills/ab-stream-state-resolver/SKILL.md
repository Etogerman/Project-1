...
- ready PR, который ждёт пользовательский merge;
...
Merge в `staging` или `main` выполняет пользователь. Агент может проверять до или
после пользовательского merge только по отдельной команде.
