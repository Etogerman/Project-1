...
Для опубликованного draft PR авторская самопроверка окончательного снимка,
заголовка, описания и контрольных полей должна быть завершена до передачи пользователю на
проверку CI. Если CI того же снимка зелёный и blocker-ов нет, следующий шаг —
пользовательский `ready`; не вставляй отдельный вызов агента только ради
подтверждения зелёного CI. Возвращай задачу агенту до `ready` только при красном
или неоднозначном CI, изменившемся снимке либо недоказанной самопроверке
текущего снимка.
