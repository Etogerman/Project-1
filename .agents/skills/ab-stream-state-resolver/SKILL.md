...
- ready code/runtime PR, который ждёт самостоятельную проверку агента, проверку
  стороннего ИИ-ревьюера того же `HEAD`, сводный технический вердикт агента или
...
независимых проверок одного зафиксированного `HEAD`: агент самостоятельно
проверяет diff без предварительного чтения выводов стороннего ИИ-ревьюера, а
сторонний ИИ-ревьюер параллельно проверяет тот же commit. После завершения обеих
проверок агент читает внешний review и все актуальные comments/threads и даёт
...
к одному текущему `HEAD`. Если одна проверка отсутствует, относится к другому
commit либо review status, comments/threads или CI status недоступны, неполны
или противоречат друг другу, следующий шаг — получить данные или решение
пользователя. Новый commit обнуляет пакет и возвращает поток к пользовательской
проверке CI текущего `HEAD`, повторной явной делегации агенту и повторной
передаче стороннему ИИ-ревьюеру. Merge в `staging` или `main` выполняет
