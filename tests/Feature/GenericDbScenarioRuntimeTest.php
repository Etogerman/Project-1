...
class GenericDbScenarioRuntimeTest extends TestCase
{
    ...
    public function test_v3_ai_analysis_unknown_output_falls_back_to_next_processor(): void
    {
        Http::fake([
            'https://api.telegram.org/*' => Http::sequence()
        ...
        $gemini
            ...
            ->andReturn(
                ...
                new AiProviderStructuredResult(
                    provider: 'gemini',
                    model: 'fallback-model',
                    parsedPayload: ['output_id' => 'name_accepted', 'data' => ['geo_city' => 'Москва']],
                    requestBodyRaw: '{}',
                    responseBodyRaw: '{}',
                    httpStatus: 200,
                    inputTokens: 11,
        ...
        data_set($schema, 'builder_v3_runtime.blocks.accepted.actions', []);
        $scenario = $this->createPublishedScenario('v3_ai_unknown_output_fallback', $schema);
        ...
        $this->assertSame('error', $request->attempts[0]->status);
        $this->assertStringContainsString('unknown output_id', (string) $request->attempts[0]->error_message);
        $this->assertSame('success', $request->attempts[1]->status);
        $this->assertSame('fallback-model', $request->model);
        $this->assertSame($request->id, data_get($run->state_payload, 'v3.ai_analysis.ai.ai_request_id'));
        $this->assertFalse(data_get($run->state_payload, 'v3.ai_analysis.ai.error'));
        $this->assertSame('name_accepted', data_get($run->state_payload, 'v3.ai_analysis.ai.output_id'));
        $this->assertSame(['geo_city' => 'Москва'], data_get($run->state_payload, 'v3.ai_analysis.ai.data'));
    ...
    public function test_v3_ai_analysis_plain_runtime_exception_records_error_attempt(): void
    {
        Queue::fake([
            RetryScenarioV3AiAnalysisJob::class,
        ]);
        Http::fake([
            'https://api.telegram.org/*' => Http::sequence()
                ->push(['ok' => true, 'result' => ['message_id' => 9368]]),
        ]);
        ...
        $gemini
            ->shouldReceive('generateStructuredWithMetadata')
            ->once()
            ->andThrow(new RuntimeException('schema exploded'));
        $this->app->instance(GeminiApiService::class, $gemini);
        ...
        $scenario = $this->createPublishedScenario('v3_ai_plain_runtime_exception', $schema);
        ...
        $this->assertCount(1, $request->attempts);
        $this->assertSame('error', $request->attempts[0]->status);
        $this->assertStringContainsString('schema exploded', (string) $request->attempts[0]->error_message);
        ...
        Queue::assertNotPushed(RetryScenarioV3AiAnalysisJob::class);
    }

    public function test_v3_ai_analysis_new_inbound_cancels_pending_retry_and_stale_retry_job_noops(): void
    {
        Queue::fake([
            RetryScenarioV3AiAnalysisJob::class,
        ]);
        Http::fake([
            'https://api.telegram.org/*' => Http::sequence()
                ->push(['ok' => true, 'result' => ['message_id' => 9369]]),
        ]);
        ...
        $scenario = $this->createPublishedScenario('v3_ai_retry_cancelled_by_new_inbound', $schema);
        ...
        $run->refresh();
        $firstRetry = data_get($run->state_payload, 'v3.ai_analysis_retry.ai');
        $firstAiRequestId = (int) data_get($firstRetry, 'ai_request_id');
        $firstToken = (string) data_get($firstRetry, 'token');

        $this->assertSame(2, data_get($firstRetry, 'cycle'));
        $this->assertSame(AiRequest::STATUS_RETRYING, AiRequest::query()->findOrFail($firstAiRequestId)->status);
        ...
        $run->refresh();
        $requests = AiRequest::query()->orderBy('id')->get();
        $secondRetry = data_get($run->state_payload, 'v3.ai_analysis_retry.ai');
        $secondAiRequestId = (int) data_get($secondRetry, 'ai_request_id');

        $this->assertCount(2, $requests);
        $this->assertSame(AiRequest::STATUS_CANCELLED, $requests[0]->status);
        $this->assertSame($firstAiRequestId, $requests[0]->id);
        $this->assertSame(AiRequest::STATUS_RETRYING, $requests[1]->status);
        $this->assertSame($secondAiRequestId, $requests[1]->id);
        $this->assertNotSame($firstToken, data_get($secondRetry, 'token'));
        $this->assertSame($secondAnswer->id, data_get($secondRetry, 'message_id'));
        Queue::assertPushed(RetryScenarioV3AiAnalysisJob::class, 2);
        ...
        $this->assertSame(AiRequest::STATUS_CANCELLED, $requests[0]->status);
        $this->assertSame(AiRequest::STATUS_RETRYING, $requests[1]->status);
        $this->assertSame($secondAiRequestId, data_get($run->state_payload, 'v3.ai_analysis_retry.ai.ai_request_id'));
        $this->assertSame($secondAnswer->id, data_get($run->state_payload, 'v3.ai_analysis_retry.ai.message_id'));
    }
    ...
    public function test_v3_change_field_action_writes_and_clears_manual_values(): void
    {
        $channel = $this->createTelegramChannel();
        [$contact, $identity, $dialog] = $this->createDialogContext($channel);
        $contact->forceFill([
            'first_name' => 'Герман',
            'first_name_source' => Contact::FIRST_NAME_SOURCE_AUTO,
            'first_name_resolution_method' => Contact::FIRST_NAME_RESOLUTION_METHOD_SCENARIO_DIRECT,
            'gender' => 'male',
        ])->save();

        $scenario = $this->createPublishedScenario('v3_change_field_manual', $this->v3ChangeFieldManualRuntimeSchema($channel->id));
    ...
    public function test_v3_change_field_action_can_store_entrypoint_parameter(): void
    {
        $channel = $this->createTelegramChannel();
        [$contact, $identity, $dialog] = $this->createDialogContext($channel);
        $scenario = $this->createPublishedScenario('v3_change_field_start_parameter', $this->v3ChangeFieldStartParameterRuntimeSchema($channel->id));
        ...
        $startMessage = Message::factory()->create([
            ...
            'sent_by_type' => Message::SENT_BY_TYPE_CONTACT,
            'external_chat_id' => $dialog->external_chat_id,
            'text' => '/start referral_42',
            'message_parameter' => 'referral_42',
        ]);
        ...
        $this->assertSame('referral_42', data_get($dialog->fields_payload, 'start_param'));
        $this->assertSame('referral_42', data_get($run->state_payload, 'v3.entrypoint.parameter'));
    }
    ...
    public function test_v3_complete_data_collection_action_marks_contact_complete_before_real_bitrix24_sync(): void
    {
        ...
        $scenario = $this->createPublishedScenario('v3_complete_data_collection_bitrix_sync', $schema);
    ...
    public function test_v3_simulate_start_parameter_clears_dialog_field_after_successful_reroute(): void
    {
        Http::fake([
            'https://api.telegram.org/*' => Http::response([
                'ok' => true,
        ...
        $channel = $this->createTelegramChannel();
        [$contact, $identity, $dialog] = $this->createDialogContext($channel);
        $dialog->forceFill(['fields_payload' => ['start_param' => 'payload_42', 'other' => 'kept']])->save();
        $scenario = $this->createPublishedScenario(
            'v3_simulate_start_parameter_clear',
            $this->v3SimulateStartParameterRuntimeSchema($channel->id, clearAfterReroute: true),
        );
        ...
        $this->assertArrayNotHasKey('start_param', $dialog->fields_payload ?? []);
        $this->assertSame('kept', data_get($dialog->fields_payload, 'other'));
        $this->assertSame(ScenarioRun::STATUS_ACTIVE, $run->status);
        $this->assertSame('payload_start', $run->current_step);
    ...
    public function test_v3_simulate_start_parameter_keeps_dialog_field_when_target_changes_it(): void
    {
        Http::fake([
            'https://api.telegram.org/*' => Http::response([
                'ok' => true,
        ...
        $dialog->forceFill(['fields_payload' => ['start_param' => 'payload_42']])->save();
        $scenario = $this->createPublishedScenario(
            'v3_simulate_start_parameter_clear_changed',
            $this->v3SimulateStartParameterRuntimeSchema(
                $channel->id,
                clearAfterReroute: true,
                targetWritesNewParameter: true,
            ),
        );
        ...
        $dialog->refresh();
        ScenarioRun::query()->where('scenario_code', $scenario->code)->firstOrFail();

        $this->assertSame('new_payload', data_get($dialog->fields_payload, 'start_param'));
    ...
    public function test_v3_simulate_start_parameter_continues_when_parameter_has_no_start_block(): void
    {
        ...
        $scenario = $this->createPublishedScenario(
            'v3_simulate_start_parameter_noop',
            $this->v3SimulateStartParameterRuntimeSchema($channel->id, clearAfterReroute: true),
        );
        ...
        $this->assertSame('unknown_payload', data_get($dialog->fields_payload, 'start_param'));
    ...
    public function test_v3_edge_expression_keeps_location_source_alias_for_region_source(): void
    {
        $channel = $this->createTelegramChannel();
        [$contact, $identity, $dialog] = $this->createDialogContext($channel);
        $contact->forceFill([
        ...
        $this->assertTrue($condition->evaluate('{{contact.region_source}} == "ai"', $message));
        $this->assertTrue($condition->evaluate('{{contact.location_source}} == "ai"', $message));
        $this->assertFalse($condition->evaluate('{{contact.location_source}} == "manual"', $message));
    }
    ...
    public function test_v3_distance_to_moscow_action_uses_fresh_contact_after_same_tick_geo_write(): void
    {
        Http::fake([
            'https://api.telegram.org/*' => Http::response([
                'ok' => true,
                'result' => ['message_id' => 9702],
            ]),
        ]);
        ...
        $scenario = $this->createPublishedScenario(
            'v3_distance_after_geo_write',
            $this->v3WriteGeoThenDistanceToMoscowRuntimeSchema($channel->id),
        );
     ...
     * @return array<string, mixed>
     */
    private function v3ChangeFieldManualRuntimeSchema(int $channelId): array
    {
        return [
            'version' => 3,
            'builder_v3_runtime' => [
                    ...
                    'change_field' => [
                        ...
                        'actions' => [
                            ...
                            [
                                ...
                                'target_field' => 'region',
                                'value_source' => 'manual',
                                'manual_value' => 'Московская область',
                                'source_block_id' => '',
                                'source_field_key' => '',
                            ],
                            [
     ...
     * @return array<string, mixed>
     */
    private function v3ChangeFieldStartParameterRuntimeSchema(int $channelId): array
    {
        return [
            'version' => 3,
            'builder_v3_runtime' => [
                ...
                'blocks' => [
                    'change_field' => [
                        ...
                        'actions' => [[
                            'type' => 'change_field',
                            'target_scope' => 'dialog',
                            'target_field' => 'start_param',
                            'value_source' => 'start_parameter',
                            'manual_value' => '',
                            'source_block_id' => '',
                            'source_field_key' => '',
                        ]],
     ...
     * @return array<string, mixed>
     */
    private function v3SimulateStartParameterRuntimeSchema(
        int $channelId,
            ...
            'builder_v3_runtime' => [
                ...
                'blocks' => [
                    ...
                    'payload_start' => [
                        ...
                        'actions' => $targetWritesNewParameter ? [[
                            'type' => 'variables',
                            'operations' => [[
                                'operation' => 'set',
                                'field_key' => 'start_param',
     ...
     * @return array<string, mixed>
     */
    private function v3WriteGeoThenDistanceToMoscowRuntimeSchema(int $channelId): array
    {
        $writeGeoEdge = [
            'id' => 'edge_write_geo_to_distance',
            'edge_key' => 'edge_write_geo_to_distance',
            'mode' => 'automatic',
            'priority' => 10,
            'transition_limit' => 0,
            'source_block_id' => 'write_geo',
            'target_block_id' => 'distance',
            'from_output_id' => null,
            'label' => 'Дальше',
        ];
        $resolvedEdge = [
            'id' => 'edge_distance_resolved_after_write',
            'edge_key' => 'edge_distance_resolved_after_write',
            'mode' => 'action_result',
            'priority' => 10,
            'transition_limit' => 0,
            'source_block_id' => 'distance',
        ...
        $pendingEdge = [
            'id' => 'edge_distance_pending_after_write',
            'edge_key' => 'edge_distance_pending_after_write',
            'mode' => 'action_result',
            'priority' => 10,
            'transition_limit' => 0,
            'source_block_id' => 'distance',
            'target_block_id' => 'pending',
            'from_output_id' => 'distance_pending',
            'label' => 'Ждёт данных',
        ];
            ...
            'builder_v3_runtime' => [
                ...
                'blocks' => [
                    'write_geo' => [
                        ...
                        'buttons' => null,
                        'wait_reply_edges' => [],
                        'automatic_edges' => [$writeGeoEdge],
                        'action_result_edges' => [],
                        'default_target_block_id' => 'distance',
                    ],
                    'distance' => [
                        'id' => 'distance',
                        'db_id' => 11,
                        ...
                        'wait_reply_edges' => [],
                        'automatic_edges' => [],
                        'action_result_edges' => [$resolvedEdge, $pendingEdge],
                        'default_target_block_id' => null,
                    ],
                    'done' => [
                        'id' => 'done',
                ...
                'edges' => [$writeGeoEdge, $resolvedEdge, $pendingEdge],
            ],
        ];
    }
