...
- `Ready for review`, `merge`, `rebase`, `force-push`, `migrate` и `deploy`
...
- такая делегация не распространяется на `Ready for review`, `merge`,
  `rebase`, `force-push`, действия после конфликтов и запуск тестов,
...
- `Ready for review` и `merge` всегда требуют отдельного явного выбора
  пользователя
