...
- при красном `CI`, конфликте, неожиданном не-документационном diff
  ...
  следующий шаг
- special-case для ТЗ:
  - изменения только в `docs/tz/**` и `docs/tz/archive/**` всегда ведутся
    напрямую в `main` по документальному пути
  - для них не используются `staging`, deploy-check и smoke
  - если diff смешан с любыми файлами вне `docs/tz/**`,
    special-case выключается и действует общий процесс
