...
18. Существенный stream по умолчанию требует versioned ТЗ до продолжения code stream.
19. Для существенного stream-а versioned ТЗ может храниться:
   - в `docs/` основного репозитория
   - во внешнем spec-repo как `Внешнее ТЗ`
20. Если stream ведётся по внешнему ТЗ, перед продолжением реализации должен быть зафиксирован `Spec revision`.
21. В основном репозитории не требуется хранить полную копию внешнего ТЗ, если для текущего stream явно зафиксирована ссылка на spec-doc и `Spec revision`.
...
- внешний spec-repo допустим как место хранения существенных ТЗ, если для stream явно зафиксирована `Spec revision`
