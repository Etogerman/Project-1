...
7. Если пользователь спрашивает о следующем логичном и правильном шаге, агент называет следующий delivery-уровень потока, если текущий уровень уже закрыт и blocker-ов нет.
8. Если следующий delivery-уровень требует отдельной явной делегации, агент рекомендует именно этот шаг процесса и отдельно указывает, что для исполнения нужна команда пользователя.
9. Если по ходу работы появляется двусмысленность, blocker, scope drift или нехватка данных, агент обязан остановиться и запросить следующий шаг.
10. Каждое сообщение агента содержит короткое меню с пронумерованными вариантами следующих действий. Формат рекомендации: `Рекомендация: 1. ...` и отдельной строкой `Почему: ...`.
11. Подробный delivery-playbook, уровни делегации и procedural flow описаны в `docs/task-delivery-workflow.md`; `AGENTS.md` фиксирует только верхнеуровневые рамки, инварианты и ограничения.
12. Существенный stream по умолчанию требует versioned ТЗ во внешнем spec-repo до продолжения code stream.
13. Перед продолжением реализации для существенного stream-а должны быть явно зафиксированы `Spec repo`, `Spec doc` и `Spec revision`.
14. Основной репозиторий хранит код, process-docs, policy-docs, reference-docs и ссылки на внешние ТЗ.
15. Если код, локальные документы и внешнее ТЗ расходятся, агент сначала показывает расхождение пользователю и не придумывает «правильную» версию молча.
