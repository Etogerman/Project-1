...
Термины:
- `ТЗ` — согласованный контракт текущего stream. Для мелких исправлений и коротких локальных действий ТЗ может фиксироваться в чате. Для существенных stream-ов ТЗ должно быть сохранено в `docs/` до продолжения реализации.
...
18. Существенный stream по умолчанию требует сохранённого ТЗ в `docs/` до продолжения code stream. К существенным stream-ам относятся как минимум staging-based rollout, multi-step implementation, изменения с runtime/data-flow/delivery риском и новые интеграционные модели.
