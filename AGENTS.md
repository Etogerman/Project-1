...
16. Если агент предлагает следующий шаг, ответ всегда оформляется как меню. Даже если допустим только один вариант, он всё равно показывается как меню из одного пункта. Варианты должны быть пронумерованы. Формат рекомендации: `Рекомендация: 1. ...` и отдельной строкой `Почему: ...`.
