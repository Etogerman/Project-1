...
33. После любого локального изменения файлов агент обязан явно сообщить:
   - какие файлы изменены
   - опубликованы ли изменения или остаются только локальным diff
   - является ли это новым хвостом stream-а
34. Если локальный diff создан намеренно, acceptance текущей локальной точки закрыт и blocker-ов нет, агент обязан рекомендовать следующий delivery-шаг процесса, а не останавливаться на констатации локального статуса.
35. Для docs-only, policy-only и reference-only изменений следующим шагом по умолчанию считается отдельный docs-only stream: commit -> draft PR, если пользователь явно не выбрал revert, pause или отказ от публикации.
36. Если публикация требует отдельной команды пользователя, агент всё равно обязан назвать именно следующий publish-шаг и отдельно указать: `Для исполнения нужна команда пользователя.`
