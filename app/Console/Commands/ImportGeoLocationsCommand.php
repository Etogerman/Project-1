...
class ImportGeoLocationsCommand extends Command
{
    protected $signature = 'geo:import-locations
        {file : Путь к geo_locations.csv}
        {--dry-run : Проверить файл без записи}
        {--delimiter=; : Разделитель CSV}';

    protected $description = 'Импортировать страны, регионы и города из CSV.';

    public function handle(GeoCsvImportService $service): int
    {
        $report = $service->importLocations(
